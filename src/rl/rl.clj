...
;;; action with value next state and reward. I.e

{<state1> {<action1> {<next-state1> <reward1>...} <action2>...} <state2>....}
...
(defn next-state [state action table]
  (-> table state action first))

(defn immediate-reward [table state action]
  (-> table state action second))
...
;;; NB: Q-Table is assumed to be an atom.
...
;;; Creates a grid world (i.e. Q-Table) using the following representation:

;;; Grid world is a map keyed by state. Each state entry is a map keyed by
